...
HEIGHT = 600
FPS = 60
SIZE_BULLET = 4.5 # Kích thước viên đạn nổ ra
...
image = pygame.image.load("assets/win_end.png")
...
class Random():
	...
	def randomBulletFlyUp_x(): # Vị trí viên đạn bay lên
		x = random.randint(int(WIDTH*0.2), int(HEIGHT*0.8))
		return x
...
def main():
	...
	while True:
		...
		new_image = pygame.transform.scale(image, (int(image.get_width()*scale//2), int(image.get_height()*scale//2)))
		rect = new_image.get_rect(center=(DISPLAYSURF.get_width() // 2, DISPLAYSURF.get_height() // 2))
